;(function(){
	...
	function Canvas(){
        ...
        this.createPiChart = function(dataStack){
			...
			for (var i=0; i<dataStack.length; i++){
				data.push(dataStack[i].score);
			}

			console.log(data);

			var colors = ["#7cf64b","#7E3817", "#C35817", "#EE9A4D", "#A0C544", "#348017", "#307D7E"];
			...
			var lastPosition = 0, total = 0;
 			for(var i in data) { total += data[i]; 
				...
				for (var i = 0; i < data.length; i++) {
					...
					var outerRadius = lastPosition+(Math.PI*2*(data[i]/total))
					ctx.beginPath();
	...
	var DataHandler = function() {
		...
		this.addAction = function(){
			...
			var data = {subject:data1, score:parseInt(data2)};
